...
class TorFileDownloader(object):
    """
    ...
    def __init__(self, tor_instance: TorInstance=None, tor_port: int=9051,
        ...
        # Logger 설정 (FileDownloader와 동일한 방식)
    ...
    def _get_url_filename(self, url: str, session: requests.Session) -> str:
        ...
        try:
            ...
            self.logger.error("Error Connecting:%s | URL: %s", errc, url)
            raise errc
        except requests.exceptions.Timeout as errt:
            self.logger.error("Timeout Error:%s | URL: %s", errt, url)
            raise errt
        except requests.exceptions.RequestException as err:
            self.logger.error("Oops, Something Else: %s | URL: %s", err, url)
            raise err
    ...
    def _check_local_file(self, filename: str, chunk_size: int, full_path: str, header: Dict=None) -> Tuple[Dict, int]:
        ...
        if os.path.isfile(full_path):
            ...
            self.logger.info("Found file '%s' in output directory, resuming download after %d chunks", filename, original_file_chunks)
    ...
    def download_file(self, url: str, target_dir: str=None, filename: str=None, chunk_size: int=1024) -> str:
        ...
        if self.num_retries >= self.max_retries:
            self.logger.error("Max retries (#%d) exceeded for URL: %s", self.num_retries, url)
            return None
        self.num_retries += 1

        self.logger.debug("Starting download from URL: %s", url)
        ...
        if target_dir and not target_path.is_dir():
            ...
            try:
                ...
                self.logger.warning("Directory '%s' does not exist, attempting to create it.", target_dir)
                target_path.mkdir(parents=True, exist_ok=True)
                self.logger.info("Directory '%s' successfully created.", target_dir)
            except OSError as err:
                self.logger.error("Error creating target_dir: %s", err)
        ...
        # Add User-Agent header to resume_header
        resume_header.update(DEFAULT_HEADERS)
        try:
            ...
            if req.status_code == 404:
                self.logger.error("Recieved 404, recheck download links. 404 link: %s", url)
                raise LinkError(f"404 for URL: {url}")
            if req.status_code == 416:
                self.logger.info("Received 416 response, assuming download is done for file: %s", filename)
            ...
            self.logger.warning("Request error, trying again. URL: %s | Error: %s", url, err)
            return self.download_file(url, target_dir=target_dir, filename=filename, chunk_size=chunk_size)
        num_bars = int(file_size) // chunk_size

        self.logger.debug("Got request: %s | Number of chunks: %d", req, num_bars)
        ...
        if int(file_size) < chunk_size:
            ...
            self.logger.info("Last bytes have been downloaded (%s bytes)!  URL: %s | Filepath: %s", file_size, url, target_dest_dir)
            return target_dest_dir
        # If the file size is 0, there is nothing to download
        if file_size == 0:
            self.logger.info("No more bytes left to download!  URL: %s | Filepath: %s", url, target_dest_dir)
            return target_dest_dir
        # If the file is not in the output directory, create it and start a stream download
        if not resume_header:
            self.logger.info("File not found in output directory, creating new file: %s", target_dest_dir)
        ...
        # If the file is in the output directory, start a stream download and append to file
            ...
            self.logger.info("File found in output directory, resuming download of file: %s", target_dest_dir)
            with open(target_dest_dir, 'ab') as output_file:
        ...
        self.logger.debug("Finalizing file: URL: %s | File size: %s | Expected file size: %s | Difference: %d",
                     url, target_file_size, file_size, target_file_size - int(file_size))
        if target_file_size != int(file_size):
            self.logger.warning("Target file size (%d) does not match expected file size (%d), restarting...", target_file_size, int(file_size))
            return self.download_file(url, target_dir=target_dir, filename=filename, chunk_size=chunk_size)
        self.logger.info("File downloaded! Elapsed Time: %s | URL: %s | Filepath: %s", str(self.start_time - datetime.now()), url, target_dest_dir)
        return target_dest_dir
